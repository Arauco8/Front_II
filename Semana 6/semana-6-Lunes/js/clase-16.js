...
function consultaAsincrona(texto) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (texto === "endpoint") {
            ...
            reject({ mensaje: "Fallo la peticion" })
        }
...
fetch(url)
    ...
    .then(dataParseada => {
        console.log(dataParseada.message)
    })
...
document.querySelector('div.comentario').remove()

function renderizarElementos(listado) {
    // desarrollar la funcion 👇
    const resultado = listado.map(item => {
        return `
    ...
    document.querySelector('div.comentarios').innerHTML = ''
    resultado.forEach(element => {
        document.querySelector('div.comentarios').innerHTML += element
    });
