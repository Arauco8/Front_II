...
// let bodyClase = document.querySelector(".vistaDeUsuario")
// let bodyId = document.querySelector("#vistaDeUsuario")

// let bodyPorClase = document.getElementsByClassName("vistaDeUsuario")
// let bodyPorId = document.getElementsById("vistaDeUsuario")

// let respuestaUsuario = confirm("¿Desea cambiar de modo?")
...
//declaramos la accion/funcion
const cambiarModo = () => {
    body.classList.toggle("dark")
}

//asociamos el boton seleccionado al evento
//como primer parametro elegimos el tipo de evento, como sdo parametro la funcion 
...
for (let h2 of title) {
    ...
    function pedirNombre(texto) {
        if (h2.textContent.includes(texto)) {
            h2.style.color = "red"
            h2.style.fontSize = "30px"
        }
    }
    pedirNombre("El tigre")
    pedirNombre("El leopardo")
    pedirNombre("El jaguar")
}
...
const [sinPeligro, enPeligro] = document.querySelectorAll(".contenedor-sin-peligro, .contenedor-peligro")

listadoFelinos.forEach(felino => {
    if(felino.peligroDeExtincion){
        enPeligro.innerHTML += `
        <div class='enPeligro'}>
        ...
        sinPeligro.innerHTML += `
        <div class='sinPeligro'}>
              <img src='${felino.imgUrl}'>
              <div>
                <h2>${felino.title}</h2>
                <p>${felino.description}</p>
